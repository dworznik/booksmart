...
# A page of a book carries a page of text. Fixtures used to carry a line, which
# the router now reads as a scan: it probes characters per page and sends a PDF
# under the threshold down the OCR route. Prose rather than
# lorem ipsum, and deliberately heading-free, so the volume changes and nothing
# a test counts does.
PAGE_FILLER = (
    "Ordinary body prose, enough of it that a page reads as a page of a book "
    "rather than as a scan with no text layer. It says nothing a test asserts "
    "on and carries no heading of its own. " * 2
)

# Always inserted at the same size as the fixture's body text, never smaller.
# The parser reads the most common size by character mass as the body size and
# promotes anything larger to a heading — so filler even one point smaller makes
# the real body text the *largest* thing on the page, and a fixture with two
# heading levels silently grows a third.
BODY_POINTS = 11
...
def make_pdf_bytes(text: str = EXTRACT_TEXT) -> bytes:
    doc = pymupdf.open()
    page = doc.new_page()
    page.insert_text((72, 72), text, fontsize=BODY_POINTS)
    page.insert_textbox(pymupdf.Rect(72, 200, 520, 700), PAGE_FILLER, fontsize=BODY_POINTS)
