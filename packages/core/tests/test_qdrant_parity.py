"""Parity between embedded local-mode Qdrant and a Qdrant server.
...
# The sparse model these points are locked to. Named once because the recipe is
# read back off it, and the collection refuses a mismatch.
...
def seed_book(
    ...
    # A sparse vector beside every dense one, because upserting a point names the
    # complete set of its vectors — writing one alone would strip the other off.
    # Parity is a dense-search property, so the lexical half only has to exist and
    # be identical on both backends; the fake gives that deterministically.
    sparse_provider = FakeSparseEmbeddingProvider(model=SPARSE_MODEL)
    sparse_vectors = sparse_provider.embed_documents(
        [f"{record_type} text" for record_type in vectors]
    )
    records = [
        VectorRecord(
            id=str(uuid.uuid4()),
        ...
        for (record_type, vector), sparse_vector in zip(
            vectors.items(), sparse_vectors, strict=True
        )
    ]
    return ids, records
...
def corpus(
    ...
    for store in (local_store, server_store):
        recipe = FakeSparseEmbeddingProvider(model=SPARSE_MODEL).recipe
        store.replace_book_points(str(book_a), records_a, QueryEmbedder.model, recipe)
        store.replace_book_points(str(book_b), records_b, QueryEmbedder.model, recipe)
...
def search_both(
    ...
    Dense explicitly, not by omission. What this suite compares is backend
    behaviour — which points each admits, and the cosine scores and thresholds
    that decide them — and those assertions are dense semantics: under fusion a
    score is a sum of reciprocal ranks and ``--score-threshold`` moves to the
    dense branch, so the same assertions would be checking a different number.
    Hybrid's own local-vs-server hazard (a root-level filter that embedded
    Qdrant silently ignores) is covered by the fusion tests that introduced it.
    """
    kwargs.setdefault("mode", "dense")
