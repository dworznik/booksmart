...
from .test_ingestion_api import BODY_POINTS, PAGE_FILLER, register_book
...
def make_structured_pdf_bytes() -> bytes:
    """Font sizes chosen so the parser emits '#' for chapters and '##' for sections.

    The filler at the foot of each page is what keeps the page above the router's
    text-layer threshold, and it is set at ``BODY_POINTS`` — the same size as the
    body text — on purpose. Body size is the most common size by character mass,
    so filler any smaller would make this fixture's 11pt body text the largest
    thing on the page and grow a third heading level nobody asked for.
    """
    doc = pymupdf.open()
    for chapter, sections in BOOK_OUTLINE:
        ...
        for section in sections:
            ...
            y += 30
            page.insert_text(
                (72, y), "Body text explaining the idea in plain words.", fontsize=BODY_POINTS
            )
            y += 40
        page.insert_textbox(pymupdf.Rect(72, 560, 520, 760), PAGE_FILLER, fontsize=BODY_POINTS)
...
class TestStructureKinds:
    def test_front_and_back_matter_distinguishable(
        ...
        for heading in ("Contents", "Preface", "Chapter One: Modules", "Index"):
            ...
            page.insert_text((72, 130), "Body text in regular size.", fontsize=BODY_POINTS)
            page.insert_textbox(
                pymupdf.Rect(72, 160, 520, 700), PAGE_FILLER, fontsize=BODY_POINTS
            )
