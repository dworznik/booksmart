...
def _strip_emphasis(title: str) -> str:
    """Remove Markdown emphasis markers so persisted titles are plain text.

    Not a workaround. An EPUB `<h2>` may legitimately contain `<em>` or
    `<strong>`, and the extractor carries that into the artifact faithfully —
    the GFM says what the book set. A *title* is a string, though: it is matched
    against a hand-authored ToC by normalised text, and `The *Open* Principle`
    would never join `The Open Principle`.
    """
    previous = None
