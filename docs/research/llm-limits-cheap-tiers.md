# LLM limits for the cheap tiers — findings
...
>
> **Frozen** (2026-07-21), when this research merged with its implementation.
...
up would be a feature, not part of this work; if it ever is wired up, these `None`s become
...
2. **`llm.py:274-276` — the `extra_body` comment's rationale is stale.**
   ...
   for this work but adjacent; flagging as asked.)
...
- **Anthropic `effort` value sets are not recorded here.** Since `AnthropicProvider`
  ...
  that is a cross-check, not a vendor citation, and it is not needed here.)
