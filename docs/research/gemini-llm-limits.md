...
>
> **Frozen** (2026-07-21), the state of the code this research was gathered against and
> describes throughout; the entries it recommends landed with it.
> Every number below is a live-API observation on that date; Gemini's
...
the 64-token budget went to reasoning. That is the trap the cheap-tier research recorded
...
- **65,536 is declared, not observed.** As with the OpenAI figures, confirming
...
- **A keyed drift test would settle this class of question permanently.** The
  cheap-tier research floated the same idea for Anthropic's Models API. The Gemini case is
