...
foreground loop; booksmart-api wraps each stage in an Inngest step and lets
Inngest's re-invocation/memoization cycle provide durability and retry.
...
semantics (Inngest's NonRetriableError, a CLI error message) without parsing
strings.

Core stays synchronous on purpose. Inngest's "async" is durable re-invocation
...
- One transaction per pipeline run (status quo) — rejected: a durable
  ...
  atomicity is unimplementable under Inngest; keeping it CLI-only would
...
- Async-first core — rejected: drags aiosqlite/asyncpg and async SDK
  ...
  cleanly in Inngest steps and threadpools.
- Core-owned shared runner — rejected: the api's runner *is* Inngest;
