...
## The deployment shape (for a server consumer)
...
- Startup ran migrations then the server:
  ...
  A server consumer runs core's single alembic history (`packages/core/migrations`,
  a second alembic environment layers its api-only tables)
...
## The compose e2e smoke test (for a server consumer / CLI e2e)
