...
Status codes and semantics below are the ones the FastAPI app served before the
split.
...
| Method | Path | Originally | Semantics |
| --- | --- | --- | --- |
...
the queued state, so the endpoint ended up running the pipeline synchronously.
